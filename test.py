...
try:
    ...
    labels = []
    docs = []
    docs_y = [] # what intent is part of

    for intent in data['intents']:
        for pattern in intent['patterns']:
            #reduce pattern to root words
...
net = tflearn.fully_connected(net, 8) # 8 neurons for hidden layer
net = tflearn.fully_connected(net, len(output[0]), activation='softmax') # getting probabilities for output
